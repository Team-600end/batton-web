...
export default function RnoteButton(props: RnoteButtonProps) {
  ...
  return (
    <button
      onClick={() => navigate(`/project/${projectKey}/releasesnote/${props.release.releasesId}`)}
    ...
    >
      ...
      {props.release.isPublished == "UNPUBLISH" && <p className="text-sm text-gray-400 font-suitM">{"발행 대기중"}</p>}
      ...
      <p className="text-xl font-suitSB text-black mr-[1.5vw] ml-[1vw]">{props.release.createdDate}</p>
