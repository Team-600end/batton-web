  ...
  return (
    <>
      <div className="flex flex-row h-[38px] w-[15vw] m-auto items-center justify-center text-green-700 bg-white border border-green-500 font-suitM rounded-lg hover:bg-gray-50 text-sm px-5 py-2.5 mt-2">
        <div>
          <img
            className="w-6 h-6 rounded-full object-cover select-none pointer-events-none"
            src={props.member.profileImage == null || props.member.profileImage == "" ? default_profile : props.member.profileImage}
            alt="avatar"
          />
        </div>
        ...
        {isLeader && (
          <button id="dropdownMenuIconButton" data-dropdown-toggle="dropdownDots" type="button" onClick={showModal}>
            <img className="w-[16px] h-[16px] rounded-full object-cover m-auto" src={vertical_dots_img} alt="vertical_dots" />
          </button>
        )}
      </div>
